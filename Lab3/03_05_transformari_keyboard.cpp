...
float xMin = -400.0f, xMax = 400.0f, yMin = -300.0f, yMax = 300.0f;		//	Variabile pentru proiectia ortogonala;

int direction_Ox = 1; /// Directia de deplasare pe axa Ox ('1' -> dreapta, '-1' -> stanga);

void ProcessNormalKeys(unsigned char key, int x, int y)
{
	switch (key) { /// Procesarea tastelor 'l' si 'r' modifica directia de deplasare a patratului pe axa Ox ('l' -> stanga, 'r' -> dreapta);
	case 'l':
		direction_Ox = -1; /// '-1' schimba directia de deplasare a patratului pe axa Ox in STANGA;
		break;
	case 'r':
		direction_Ox = 1; /// '1' schimba directia de deplasare a patratului pe axa Ox in DREAPTA;
		break;
	}

	if (key == 27) /// If "Esc" key is pressed
		exit(0); /// The program will be successfully terminating without any errors or interruptions.
}
...
void CreateVBO(void)
{
	...
	GLfloat Colors[] = {
		...
		1.0f, 0.0f, 1.0f, 1.0f, // Purple (Magenta);
...
///  (Procesata intr-un "loop" in functia "Main" pana cand user-ul inchide fereastra);
...
{	
	...
	codCol = 1;								//	Schimbare culoare (Red);
	glUniform1i(codColLocation, codCol);
	glDrawArrays(GL_POLYGON, 8, 4);
	codCol = 2;								//	Schimbare culoare (Blue);
	glUniform1i(codColLocation, codCol);
